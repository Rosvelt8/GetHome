    ...
    if(!empty($_POST['locationDescription']) AND !empty($_POST['locationAmount']) AND !empty($_POST['locationSpace']) AND !empty($_POST['locationType']) AND !empty($_POST['locationStatus']) AND !empty($_POST['locationPlace']) AND !empty($_POST['locationSalon']) AND !empty($_POST['locationDouche']) AND !empty($_POST['locationChambre']) AND !empty($_POST['locationCuisine'])){
        ...
        if(move_uploaded_file($_FILES['locationFirstPicture']['tmp_name'], $target)){
            if(move_uploaded_file($_FILES['locationSecondPicture']['tmp_name'], $target2)){
                $insertLocationOnWebsite=$bdd->prepare("INSERT INTO `location` (`id_location`, `id_agent`, `first_image`, `second_image`, `type`, `statut`, `description`, `localisation`, `prix`, `chambre`, `douche`, `salon`, `cuisine`, `superficie`) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");

                $insertLocationOnWebsite->execute(array($id_locator_actor,$location_picture_one, $location_second_one,$location_type, $location_Status, $location_description, $location_place, $location_amount, $location_chambre, $location_douche, $location_salon, $location_cuisine,$location_space));

                $successMessage="votre location a bien été publié dans le site";
